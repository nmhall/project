...
local function SafeTry(func,...)
	local Get=function(func) return loadstring("return "..func)() end
	local f=Get(func)
	if type(f)~="function" then
		Spring.Echo(func.." not supported!")
	else
		local status, msg = pcall(f,...)
		if not status then
			Spring.Echo(func.." failed: "..msg)
		end
	end
end
...
local function DoTheRestart(startscriptfilename)
	...
	complete,_=string.gsub(complete,"([\r\n]+[ \t]*HostIP=)[ \t]*localhost[ \t]*;","%1;",1)-- Since 0.82.7 HostIP=localhost; makes Spring crash
	...
	file:flush()
	file:close()
	Spring.Echo(widget:GetInfo().name..": Ok, calling Spring.Restart(\"\",\"[GAME]{..}\") now!")
	Spring.Restart("",trimmed)
	Spring.Echo(widget:GetInfo().name..": Just called Spring.Restart(\"\",\"[GAME]{..}\")")
...
local function ListMap(ReturnTo)
	...
	for m,_ in ipairs(ModSpecific.MapsList) do
		...
		AddFrame(ModSpecific.MapsList[m].ClearName..(missing and " - Missing!" or ""),
			{x=vsx*(0.57-0.25*math.floor((m-1)/8)),y=vsy*(0.15+0.1*((m-1)%8))},vsy/28,{0,1,0,0.5},"lc","c",
			missing and ListMap or ChangeMap,missing and ReturnTo or m,ReturnTo)
	end
...
function widget:Initialize()
	SafeTry("Spring.SetWMIcon","bitmaps/icons/SpringAppIconKP.bmp")
	SafeTry("Spring.SetWMCaption","Kernel Panic!","Kernel Panic!")
	BetterizeRandom()
	IsActive = false
